...
void getDeviceList(G31DDC_DEVICE_INFO *deviceList,int *deviceCount){
	*deviceCount		=	GetDeviceList(NULL,0);
	switch (*deviceCount){
		case -1:
			deviceList = NULL;
			*deviceCount = -1;
		...
		break;
		default:
			deviceList		=	(G31DDC_DEVICE_INFO *)malloc((*deviceCount)*sizeof(G31DDC_DEVICE_INFO));
			*deviceCount 	=	GetDeviceList(deviceList,(*deviceCount)*sizeof(G31DDC_DEVICE_INFO));
		break;
	}
}
int radio_start(const char *SN){
	if(!API){
		puts("API is not loaded");
		return 0;
	}
